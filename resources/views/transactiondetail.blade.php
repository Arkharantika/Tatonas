...
<div class="container">
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">List Data Transaction Detail</li>
                </ol>
            </nav>
        </div>
        <div class="ms-auto">
    ...
    <hr />
    <p class="mb-0 text-uppercase display-6 text-center">List Data Transaction Detail</p>
    <hr />
                        ...
                        <tr>
                            <div class="col-sm-9 text-secondary">
                                <a type="button" class="btn btn-outline-success mr-5"
                                    href="{{url('/transactiondetail/tambahdata')}}">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="edittransactiondetail/{{$row->id}}" class="btn btn-sm btn-warning mr-5 mb-5"><i
                                        class="bx bx-edit-alt"></i></a>
                                @if((Auth::user()->role ?? '') == 'super admin')
                                <a href="deletetransactiondetail/{{$row->id}}"
                                    onclick="return confirm('Yakin untuk menghapus data ini?')"
                                    class="btn btn-sm btn-danger mr-5 mb-5"><i class="bx bx-eraser"></i></a>
                                @endif
                                @if((Auth::user()->role ?? '') == 'super admin' || (Auth::user()->role ?? '') == 'admin'
                                )
                                <a href="softdeletetransactiondetail/{{$row->id}}"
                                    onclick="return confirm('Yakin untuk menghapus data ini?')"
